...
`/` はライブの MCP エンドポイントそのものです（MCP クライアントが実際に
JSON-RPC を POST/GET する先はここです）。ブラウザで単純にここへアクセスして
もデバッグ画面は出ません。それは実際のプロトコルリクエストとして扱われ、
たいていの upstream MCP サーバーはブラウザからの素の GET を拒否します
（405/406 になりますが、これはプロキシのバグではありません）。ブラウザで
見るなら代わりに `http://localhost:8080/ui` にアクセスしてください（起動時
に毎回コンソールへ出力されます）— [Live debug UI（ライブデバッグ UI）
](#live-debug-ui) を参照。
...
- **グレースフルシャットダウンは `/events` の接続が開いている間待ち続けます。**
  `/ui` を開いたままのブラウザタブは SSE 接続を維持し続けるため、uvicorn
  で `Ctrl+C` すると "Waiting for connections to close" と表示されたまま、
  そのタブが切断されるまで終了しません。先にタブを閉じるか、`Ctrl+C` を
  2 回押して強制終了してください。
